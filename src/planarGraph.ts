...
class PlanarNode extends GraphNode implements XY{
	...
	translate(dx:number, dy:number, dz?:number):XY{ var p:XY = new XY(this).translate(dx, dy, dz); return this.setPosition(p); }
	rotate90(axis?:Line):XY{ var p:XY = new XY(this).rotate90(axis); return this.setPosition(p); }
	rotate180(axis?:Line):XY{ var p:XY = new XY(this).rotate180(axis); return this.setPosition(p); }
	rotate270(axis?:Line):XY{ var p:XY = new XY(this).rotate270(axis); return this.setPosition(p); }
	rotate(angle:number, axis?:Line):XY{ var p:XY = new XY(this).rotate(angle, axis); return this.setPosition(p); }
	reflect(plane:Plane):XY{ var p:XY = new XY(this).reflect(plane); return this.setPosition(p); }
	project(plane?:Plane):XY{ var p:XY = new XY(this).project(plane); return this.setPosition(p); }
	...
	cross(vector:XY):XY{ return new XY(this).cross(vector); }
	signedArea(vector:XY):number{ return new XY(this).signedArea(vector); }
	...
	sign():number{ return new XY(this).sign(); }
	...
	commonZ(point:XY, epsilon?:number):boolean{ return new XY(this).commonZ(point, epsilon); }
...
class PlanarEdge extends GraphEdge implements Edge{
	...
	transform(matrix:Matrix):LineType{ this.nodes[0].transform(matrix); this.nodes[1].transform(matrix); return this; }
	project(plane?:Plane):LineType{ this.nodes[0].project(plane); this.nodes[1].project(plane); return this; }
	...
	equivalent(line:LineType, epsilon?:number){ return new Edge(this).equivalent(line.copy(), epsilon); }
	...
	nearestPoint(a:any, b?:number, c?:number):XY{ return new Edge(this).nearestPoint(a,b,c); }
	nearestPointNormalTo(a:any, b?:number, c?:number):XY{ return new Edge(this).nearestPointNormalTo(a,b,c); }
	midpoint():XY{ return new Edge(this).midpoint(); }
	copy():LineType{ return new Edge(this)/*.copy()*/; }
	perpendicularBisector():Plane{ return new Edge(this).perpendicularBisector(); }
...
class PlanarFace extends Polygon{
	...
	project(plane?:Plane){ this.nodes.forEach(function(node:PlanarNode){ node.project(plane); }); return this; }
}
...
class PlanarGraph extends Graph{
	...
	clean(epsilon?:number):PlanarClean{
		this.unclean = false;
		// console.time("clean");
		//project the graph to the XY plane
		this.nodes.forEach(function(el:PlanarNode){	el.project(); });
